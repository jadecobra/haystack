...
/** Sync address bar to /{TICKER} without a history entry or Next remount. */
...
export function AnalyzePage({ initialTicker }: AnalyzePageProps) {
  const seeded = initialTicker
    ? normalizeTickerSegment(initialTicker) ?? ''
    : '';
  const [ticker, setTicker] = useState(seeded);
  ...
  const runAnalyze = useCallback(
    async (rawSymbol: string) => {
      const symbol = normalizeTickerSegment(rawSymbol);
      if (!symbol) return;
    ...
    [clearWaitTimers, startWaitChrome],
  );

  useEffect(() => {
    if (!seeded || autoRanRef.current) return;
    autoRanRef.current = true;
    void runAnalyze(seeded);
  }, [seeded, runAnalyze]);
